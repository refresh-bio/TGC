...
int main (int argc, char * const argv[]) {
    ...
    int bufPos1[noGen], bufPos2[noGen];
	unsigned char ** buf1, ** buf2;
    buf1 = (unsigned char **) malloc (noGen * sizeof(unsigned char *));
    buf2 = (unsigned char **) malloc (noGen * sizeof(unsigned char *));
    for (i=0; i < noGen; i++)
    {
        buf1[i] = (unsigned char *) malloc (BUFFER_SIZE * sizeof(unsigned char));
        buf2[i] = (unsigned char *) malloc (BUFFER_SIZE * sizeof(unsigned char));
        clearBuffer(buf1[i], bufPos1[i], BUFFER_SIZE);
        clearBuffer(buf2[i], bufPos2[i], BUFFER_SIZE);
    }
    
    
    
   // FILE * resultSeq1[noGen], * resultSeq2[noGen];
    ...
    for (i=0; i < noGen; i++)
    {
        ...
        bvName[0][i] = (char*) malloc ((strlen(genomes[i])+30)*sizeof(char));
        bvName[1][i] = (char*) malloc ((strlen(genomes[i])+30)*sizeof(char));
         
        helpFileName[0]='\0';
        strcat(helpFileName, genomes[i]);
        strcat(helpFileName, ".1.");
        strcat(helpFileName, p);
        strcat(helpFileName, "\0");
        
        
        bvName[0][i][0]='\0';
        strcat(bvName[0][i], helpFileName);
        strcat(bvName[0][i], ".bv");
        strcat(bvName[0][i], "\0");
        ...
        strcat(helpFileName, genomes[i]);
        strcat(helpFileName, ".2.");
        strcat(helpFileName, p);
        strcat(helpFileName, "\0");
        
        bvName[1][i][0]='\0';
        strcat(bvName[1][i], helpFileName);
        strcat(bvName[1][i], ".bv");
        strcat(bvName[1][i], "\0");
        
        file = fopen(bvName[0][i], "wb");
        fclose(file);
        file = fopen(bvName[1][i], "wb");
        fclose(file);
...
#pragma omp parallel
    ...
    {
        tempBits1[i] = (unsigned char**)malloc(noGen * sizeof( unsigned char*));
        if(tempBits1[i] == NULL)
        {
            fprintf(stderr, "Out of memory\n");
            exit(9);
        }
        tempBits2[i] = (unsigned char**)malloc(noGen * sizeof(unsigned  char*));
        if(tempBits2[i] == NULL)
        {
            fprintf(stderr, "Out of memory\n");
            exit(9);
...
#pragma omp for
            for(int t = 0; t < noThreads; t++)
            {
                ...
                for (j = 0; j < NO_LINES_PER_THREAD; j++)
                {
                    ...
                    while (altList[t][currChar] != '\0')
                    {
                        ...
                        if (strstr(info[t], "VT=SNP") != NULL)  //VT is a SNP or chrY(only SNPs)
                        {
                            if (strlen(refC[t]) != 1)
                            {
        ...
        for (int t = 0; t < noThreads; t++)
        {
            for (int g = 0; g < noGen; g++)
            {
                for (int b = 0; b <= (bitsCalculated[t]-1)/8; b++)
                {
                    ...
                    writeBits(bvName[0][g], buf1[g], bufPos1[g], tempBits1[t][g][b], writeno);
                    
                    writeBits(bvName[1][g], buf2[g], bufPos2[g], tempBits2[t][g][b], writeno);
                }
            }
    ...
    for (i=0; i < noGen; i++)
    {
        flushBuffer(bvName[0][i], buf1[i], bufPos1[i]);
        flushBuffer(bvName[1][i], buf2[i], bufPos2[i]);
